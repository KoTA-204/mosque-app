...
class UpdateHakAksesRequest extends FormRequest
{
    ...
    {
        $hakAksesId = $this->route('hak_akses')->id;

        return [
            'kode_hak_akses' => 'bail|sometimes|string|max:100|unique:hak_akses,kode_hak_akses,' . $hakAksesId,
            'nama_hak_akses' => [
                'bail',
                'sometimes',
                'string',
                'max:100',
                new NamaMiripRule('hak_akses', exceptId: $hakAksesId),
            ],
            'modul'          => 'sometimes|string|max:100',
            'aksi'          => 'sometimes|string|max:100',
