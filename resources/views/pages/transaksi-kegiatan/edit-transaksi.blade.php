<x-modal id="modal-edit-transaksi" title="Edit Transaksi">
    ...
    <form id="form-edit-transaksi" method="POST" enctype="multipart/form-data" class="space-y-5">
        ...
        <div>
            ...
            <div class="flex rounded-xl border border-gray-200 dark:border-gray-700 overflow-hidden">
                <label class="flex-1 cursor-pointer">
                    <input type="radio" name="jenis_transaksi" value="PEMASUKAN" class="sr-only" onchange="updateEditToggleStyle('PEMASUKAN')">
                    <span id="edit-btn-pemasukan" class="block py-2.5 text-center text-sm font-medium transition-colors text-gray-500">Pemasukan</span>
                </label>
                <label class="flex-1 cursor-pointer border-l border-gray-200 dark:border-gray-700">
                    <input type="radio" name="jenis_transaksi" value="PENGELUARAN" class="sr-only" onchange="updateEditToggleStyle('PENGELUARAN')">
                    <span id="edit-btn-pengeluaran" class="block py-2.5 text-center text-sm font-medium transition-colors text-gray-500">Pengeluaran</span>
                </label>
            </div>
        </div>
        ...
        <div>
            ...
            <p id="edit-dompet-error" class="hidden mt-1.5 text-xs text-red-500">Dompet wajib dipilih.</p>
        </div>
        ...
        <div>
            ...
            <p id="edit-kategori-error" class="hidden mt-1.5 text-xs text-red-500">Kategori wajib dipilih.</p>
        </div>
        ...
        <div>
            ...
            <textarea name="deskripsi" id="edit-deskripsi" rows="3"
                      class="w-full px-4 py-2.5 text-sm border border-gray-200 dark:border-gray-700 focus:border-green-400 rounded-xl outline-none resize-none bg-white dark:bg-gray-800 text-gray-900 dark:text-white placeholder-gray-400"></textarea>
        </div>
        ...
        <div>
            ...
            <p id="edit-bukti-hint" class="hidden text-xs text-gray-400 dark:text-gray-500 mb-2">Centang ✕ untuk menghapus file lama, lalu upload yang baru di bawah.</p>
            <label class="flex cursor-pointer flex-col items-center justify-center rounded-xl border-2 border-dashed border-gray-200 dark:border-gray-700 px-4 py-8 hover:border-green-400 transition-colors">
                <input type="file" name="bukti_transaksi[]" multiple accept=".jpg,.jpeg,.png,.pdf" class="sr-only" id="editBuktiInput" onchange="showEditFileNames(this)">
        ...
        <div class="pt-1 flex items-center gap-3">
            <button type="button" onclick="validateAndSubmitEdit()"
