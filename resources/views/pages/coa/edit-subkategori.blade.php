...
<x-modal
    id="editSubKategoriModal{{ $subKategori->id }}"
    title="Edit Sub Kategori Akun"
>

<div class="p-6">

    <form method="POST" action="{{ route('dashboard.coa.sub-kategori.update', $subKategori->id) }}" class="space-y-5">
        @csrf
        @method('PUT')
        ...
        <div>
            ...
            <div class="relative">
                <select name="kategori_akun_id" class="w-full px-4 py-2.5 text-sm border rounded-xl outline-none appearance-none transition-colors
                    {{ $errors->has('kategori_akun_id') ? 'border-red-400' : 'border-gray-200 dark:border-gray-700 focus:border-green-400' }}
                    ...
                    @foreach($allKategori as $kat)
                        <option value="{{ $kat->id }}"
                            {{ old('kategori_akun_id', $subKategori->kategori_akun_id) == $kat->id ? 'selected' : '' }}>
        ...
        <div>
            ...
            <input type="text" name="kode_akun" value="{{ old('kode_akun', $subKategori->kode_akun) }}" class="w-full px-4 py-2.5 text-sm border rounded-xl outline-none transition-colors
        ...
        <div>
            ...
            <input type="text" name="nama_akun" value="{{ old('nama_akun', $subKategori->nama_akun) }}" class="w-full px-4 py-2.5 text-sm border rounded-xl outline-none transition-colors
        ...
        <div>
            ...
            <textarea name="deskripsi" rows="4" class="w-full px-4 py-2.5 text-sm border border-gray-200 dark:border-gray-700 focus:border-green-400 rounded-xl outline-none resize-none transition-colors
                bg-white dark:bg-gray-800 text-gray-900 dark:text-white placeholder-gray-400">{{ old('deskripsi', $subKategori->deskripsi) }}</textarea>
        </div>
