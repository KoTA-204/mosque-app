    ...
    <table class="w-full text-sm">
        <thead>
            <tr class="border-b border-gray-100 dark:border-gray-800">
                ...
                <th class="text-left text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider px-4 py-3">Peran</th>
        ...
        <tr class="hover:bg-gray-50 dark:hover:bg-gray-800/40 transition-colors">
            ...
            <td class="px-4 py-3.5 font-medium text-gray-800 dark:text-gray-200">{{ $item->nama }}</td>
            <td class="px-4 py-3.5 text-gray-500 dark:text-gray-400">{{ explode('@', $item->email)[0] }}</td>
            <td class="px-4 py-3.5 text-gray-500 dark:text-gray-400">{{ $item->email }}</td>
            ...
            <td class="px-5 py-3.5">
                <div class="flex items-center justify-center gap-1">
                    @php $kredensialSent = $item->kredensial_dikirim_pada; @endphp
                    <button onclick="sendCredentials({{$item->id}})"
                        title="{{$kredensialSent ? 'Kredensial sudah dikirim ' . $kredensialSent->format('d M Y H:i') . ' - klik untuk kirim ulang' : 'Kirim kredensial ke pengguna (belum dikirim)' }}"
...
<div class="flex items-center justify-between px-5 py-4 border-t border-gray-100 dark:border-gray-800 flex-wrap gap-3">
    <div class="flex items-center gap-1">
        ...
        @else
        <a href="{{ $pengguna->previousPageUrl() }}" class="px-3 py-1.5 text-sm text-gray-600 dark:text-gray-400 border border-gray-200 dark:border-gray-700 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors">Previous</a>
        @endif

        @foreach($pengguna->getUrlRange(1, $pengguna->lastPage()) as $page => $url)
        <a href="{{ $url }}" class="w-8 h-8 flex items-center justify-center text-sm rounded-lg transition-colors {{ $page === $pengguna->currentPage() ? 'bg-green-600 text-white font-medium' : 'text-gray-600 dark:text-gray-400 border border-gray-200 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-800' }}">
            {{ $page }}
        </a>
        @endforeach

        @if($pengguna->hasMorePages())
        <a href="{{ $pengguna->nextPageUrl() }}" class="px-3 py-1.5 text-sm text-gray-600 dark:text-gray-400 border border-gray-200 dark:border-gray-700 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors">Next</a>
        @else
