...
<div class="p-6 space-y-6">
    ...
    <div class="flex items-center justify-between bg-white dark:bg-gray-900 rounded-2xl border border-gray-200 dark:border-gray-800 px-6 py-4">
        ...
        <a href="{{ route('dashboard.hak-akses.edit', $hak_akses) }}"
    ...
    <div class="space-y-4">
        ...
        <div class="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-2xl p-6 space-y-5">
            ...
            {{-- Kode --}}
            <div>
                <p class="text-xs font-medium text-gray-400 dark:text-gray-500 uppercase tracking-wider mb-1">Kode HakAkses</p>
                <p class="font-mono text-sm text-gray-800 dark:text-gray-200">{{ $hak_akses->kode_hak_akses }}</p>
            </div>

            {{-- Nama --}}
            <div>
                <p class="text-xs font-medium text-gray-400 dark:text-gray-500 uppercase tracking-wider mb-1">Nama HakAkses</p>
                <p class="text-sm text-gray-800 dark:text-gray-200">{{ $hak_akses->nama_hak_akses }}</p>
            </div>
            ...
            <div class="grid grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    @php
                        ...
                        $style = $actionStyles[$hak_akses->aksi] ?? 'bg-gray-50 dark:bg-gray-800 text-gray-600 dark:text-gray-400';
                    @endphp
                    <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium {{ $style }}">
                        {{ ucfirst($hak_akses->aksi) }}
                    </span>
                </div>
            </div>
            ...
            <div>
                ...
                <span class="text-sm font-medium {{ $hak_akses->aktif ? 'text-green-600 dark:text-green-400' : 'text-red-500 dark:text-red-400' }}">
                    {{ $hak_akses->aktif ? 'Aktif' : 'Nonaktif' }}
                </span>
            </div>
            ...
            <div>
                ...
                <p class="text-sm text-gray-600 dark:text-gray-400">{{ $hak_akses->deskripsi ?? '—' }}</p>
            </div>
        </div>
        ...
        <div class="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-2xl p-6">
            ...
            <div class="flex flex-wrap gap-2">
                @forelse($hak_akses->peran as $peran)
                ...
                @empty
                    <p class="text-sm text-gray-400 dark:text-gray-600">Belum dipakai peran manapun.</p>
                @endforelse
            </div>
        </div>
