...
<div class="p-6 space-y-6">
    ...
    <div class="max-w-2xl mx-auto">
        <div class="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-2xl p-6">
            <h2 class="text-base font-semibold text-gray-900 dark:text-white mb-6">Edit Kategori</h2>

            <form method="POST" action="{{ route('dashboard.kategori-transaksi.update', $kategoriTransaksi) }}" class="space-y-5">
                @csrf @method('PUT')
                ...
                <div>
                    ...
                    <input type="text" name="nama_kategori"
                        value="{{ old('nama_kategori', $kategoriTransaksi->nama_kategori) }}"
                        ...
                        <div class="relative">
                            <select name="jenis_transaksi"
                                ...
                                <option value="PEMASUKAN"
                                    {{ old('jenis_transaksi', $kategoriTransaksi->jenis_transaksi) === 'PEMASUKAN' ? 'selected' : '' }}>
                                    Pemasukan
                                </option>
                                <option value="PENGELUARAN"
                                    {{ old('jenis_transaksi', $kategoriTransaksi->jenis_transaksi) === 'PENGELUARAN' ? 'selected' : '' }}>
                                    Pengeluaran
                                </option>
                            </select>
                        ...
                        <div class="relative">
                            <select name="status"
                                class="w-full px-4 py-2.5 text-sm border rounded-xl outline-none appearance-none transition-colors
                                    ...
                                    bg-white dark:bg-gray-800 text-gray-900 dark:text-white">
                                <option value="aktif"
                                    {{ old('status', $kategoriTransaksi->status) === 'aktif' ? 'selected' : '' }}>
                                    Aktif
                                </option>
                                <option value="tidak_aktif"
                                    {{ old('status', $kategoriTransaksi->status) === 'tidak_aktif' ? 'selected' : '' }}>
                                    Tidak Aktif
                                </option>
                            </select>
                            <svg class="pointer-events-none absolute right-3 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-400"
                ...
                <div>
                    ...
                    <textarea name="deskripsi" rows="4"
                        ...
                        class="w-full px-4 py-2.5 text-sm border border-gray-200 dark:border-gray-700 focus:border-green-400 rounded-xl outline-none resize-none transition-colors
                            bg-white dark:bg-gray-800 text-gray-900 dark:text-white placeholder-gray-400">{{ old('deskripsi', $kategoriTransaksi->deskripsi) }}</textarea>
                </div>
