        ...
        <tr class="hover:bg-gray-50 dark:hover:bg-gray-800/40 transition-colors">
            ...
            <td class="px-5 py-3.5">
                <div class="flex items-center justify-center gap-1">
                    ...
                    {{-- Edit --}}
                    @php $hasTransaksi = ($item->transaksi_count ?? 0) > 0; @endphp
                    <button <?php echo $hasTransaksi ? 'type="button" disabled aria-disabled="true"' : 'onclick="openEditModal(' . $item->id . ')"'; ?>
                        class="p-1.5 rounded-lg transition-colors <?php echo $hasTransaksi ? 'text-gray-200 dark:text-gray-700 cursor-not-allowed' : 'text-gray-400 hover:text-gray-700 dark:hover:text-gray-200 hover:bg-gray-100 dark:hover:bg-gray-700'; ?>"
                        title="<?php echo $hasTransaksi ? 'Kegiatan yang sudah memiliki transaksi tidak dapat diedit' : 'Edit'; ?>">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/></svg>
                    </button>
                    {{-- Delete --}}
                    <button <?php echo $hasTransaksi ? 'type="button" disabled aria-disabled="true"' : 'onclick="openDeleteModal(' . $item->id . ')"'; ?>
                        class="p-1.5 rounded-lg transition-colors <?php echo $hasTransaksi ? 'text-gray-200 dark:text-gray-700 cursor-not-allowed' : 'text-gray-400 hover:text-red-500 hover:bg-red-50 dark:hover:bg-red-900/20'; ?>"
                        title="<?php echo $hasTransaksi ? 'Kegiatan yang sudah memiliki transaksi tidak dapat dihapus' : 'Hapus'; ?>">
...
<div class="flex items-center justify-between px-5 py-4 border-t border-gray-100 dark:border-gray-800 flex-wrap gap-3">
    <div class="flex items-center gap-1">
        ...
        @else
        <a href="{{ $kegiatan->previousPageUrl() }}" onclick="loadPage(event, this.href)"
        ...
        <a href="{{ $url }}" onclick="loadPage(event, this.href)"
           class="w-8 h-8 flex items-center justify-center text-sm rounded-lg transition-colors {{ $page === $kegiatan->currentPage() ? 'bg-green-600 text-white font-medium' : 'text-gray-600 dark:text-gray-400 border border-gray-200 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-800' }}">
            {{ $page }}
        </a>
        @endforeach

        @if($kegiatan->hasMorePages())
        <a href="{{ $kegiatan->nextPageUrl() }}" onclick="loadPage(event, this.href)"
